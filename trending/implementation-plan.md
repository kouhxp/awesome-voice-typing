# Minimal implementation plan: Trending Voice Typing Tools
...
- [x] During implementation, add **Trending Voice Typing Tools**, its Contents link, and block markers immediately before Directory. The script replaces only the content between `<!-- trending:start -->` and `<!-- trending:end -->`. Show `Tool | Stars gained (30 days) | Total stars | Growth`, linked names, integer counts, and `Based on stars gained in the last 30 days | Last updated: YYYY-MM-DD` using the UTC run date. Sort by gained stars descending, then name case-insensitively; take `TOP_N = 5`. Growth is `gained / (current_total - gained) * 100`, rounded to one decimal, or `—` for a nonpositive denominator. If all repositories fail, leave the existing block untouched; otherwise publish available rows without coverage notes.
